...
- 🎯 Entry points to execute your program using `python -m <indexapp>` or `$ indexapp` with basic CLI argument parsing.
...
---
# indexapp Flask Application

Awesome indexapp created by scienceuli
...
```bash
git clone https://github.com/scienceuli/indexapp indexapp
cd indexapp
make install
```
...
```bash
indexapp create-db   # run once
indexapp populate-db  # run once (optional)
indexapp add-user -u admin -p 1234  # ads a user
indexapp run
```
