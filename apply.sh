...
original_author="scienceuli"
original_name="indexapp"
original_urlname="indexapp"
original_description="Awesome indexapp created by scienceuli" 
...
rm -rf "${name}"
rm -rf tests
cp -R ${TEMPLATE_DIR}/indexapp "${name}"
...
then
    ...
    echo "then run 'indexapp' or 'python -m indexapp'"
else
    poetry install
    echo "Applied Flask template"
    echo "Ensure you activate your env with 'poetry shell'"
    echo "then run 'indexapp' or 'python -m indexapp' or 'poetry run indexapp'"
fi
